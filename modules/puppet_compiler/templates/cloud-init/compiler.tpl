...
runcmd:
  ...
  - "curl -s -k https://master.${domain}:8140/packages/current/install.bash | bash -s main:dns_alt_names=puppet,puppet.${domain},lb,lb.${domain} extension_requests:pp_role=${pp_role} custom_attributes:challengePassword=${autosign_pwd} --puppet-service-ensure stopped"
